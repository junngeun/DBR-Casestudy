...
export default function HistoryPage({ onBack }) {
  ...
  const removeItem = (title) => {
    ...
    setHistory(updated);
    if (summaryCase && summaryCase.title === title) setSummaryCase(null);
  };
            ...
            {history.map((item, i) => (
              <div key={i} style={styles.card}>
                ...
                <div style={styles.cardBottom}>
                  <button style={styles.summaryBtn} onClick={() => setSummaryCase(item)}>
                    요약문 바로보기
                  </button>
                  <button style={styles.articleBtn} onClick={() => openOriginalArticle(item)}>
                    DBR 원문 바로가기 →
                  </button>
                </div>
              </div>
