    ...
    <div style={styles.page}>
      ...
      <div style={styles.logoArea}>
        <h1 style={styles.logoTitle}>
          어떤 비즈니스 문제를{" "}
          <span style={{ color: "#e85a18" }}>해결하려 하시나요?</span>
        </h1>
      </div>
        ...
        <div style={styles.categoryChips}>
          {CATEGORIES.map((cat) => (
            <button
              ...
              style={{
                ...styles.categoryChip,
                background: selectedCategory.includes(cat) ? "#E85A1B" : "#fff",
                color: selectedCategory.includes(cat) ? "#fff" : "#444",
                borderColor: selectedCategory.includes(cat) ? "#E85A1B" : "#e0e0e0",
              }}
              onClick={() => {
      ...
      <div style={styles.card}>
        <p style={styles.cardLabel}>
          ...
          {result && <span style={{ color: "#e85a18" }}>5</span>}
        </p>
...
function CaseItem({ item, isSelected, onClick }) {
  return (
    <div style={{ 
      ...styles.caseItem, 
      borderColor: isSelected ? "#e85a18" : "#e8e8e8", 
      background: isSelected ? "#FEF0E9" : "#fff",
    }} onClick={onClick}>
      <div style={{
        ...styles.caseRank,
        color: item.rank <= 3 ? "#e85a18" : "#aaaaaa",
      }}>
        {item.rank}
      </div>
      <div style={{ flex: 1 }}>
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
        ...
        {caseData.tags?.map((t) => (
          <span key={t} style={{ ...styles.caseTag, background: "#FEF0E9", color: "#e85a18" }}>{t}</span>
        ))}
      </div>
      ...
      <button
        style={{
          ...
          color: "#fff",
          background: isSelected ? "#1a1a1a" : selectedCases.length >= 3 && !isSelected ? "#ccc" : "#e85a18",
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "90%", maxWidth: 1100, maxHeight: "85vh", background: "#fff", borderRadius: 16, zIndex: 600, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: "#e0e0e0", padding: "1.5rem 2rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            ...
            <button style={{ background: "rgba(232, 90, 24, 0.1)", border: "none", borderRadius: "50%", width: 32, height: 32, fontSize: 16, color: "#e85a18", cursor: "pointer" }} onClick={onClose}>✕</button>
          </div>
        </div>
                ...
                {cases.map((c, i) => (
                  <th key={c.rank} style={{ padding: "14px 16px", textAlign: "left", background: "#fafafa", borderLeft: "1px solid #e8e8e8" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ fontSize: 16, fontWeight: 700, color: "#e85a18" }}>{i + 1}</div>
                ...
                { label: "발행일", key: "date", values: cases.map((c) => c.date ?? "-") },
                {
                  ...
                  values: cases.map((c) => (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {c.tags?.map((t) => <span key={t} style={{ padding: "2px 8px", fontSize: 11, background: "#FEF0E9", color: "#e85a18", borderRadius: 4 }}>{t}</span>)}
                    </div>
                  ))
                },
                {
                  ...
                  values: cases.map((c) => (
                    <div>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#999", marginBottom: 4 }}>
                        <span></span><span style={{ color: "#e85a18", fontWeight: 500 }}>{c.similarity}%</span>
                      </div>
                      <div style={{ height: 4, background: "#f0f0f0", borderRadius: 2 }}>
                        <div style={{ height: "100%", background: "#e85a18", borderRadius: 2, width: `${c.similarity}%` }} />
                      </div>
                    </div>
                  ))
                },
        ...
        <div style={{ padding: "1rem 2rem", borderTop: "1px solid #e8e8e8", display: "flex", justifyContent: "flex-end", gap: 8 }}>
          ...
          <button style={{ padding: "10px 20px", fontSize: 13, fontWeight: 500, color: "#fff", background: "#e85a18", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}>선택한 케이스 PDF로 내보내기</button>
        </div>
      </div>
    </>
  );
...
const styles = {
  ...
  textarea: {
    ...
    borderRadius: 12, resize: "vertical", lineHeight: 1.6,
  ...
  btnSearch: {
    ...
    color: "#fff", background: "#e85a18", border: "none",
  ...
  chip: {
    padding: "5px 12px", fontSize: 14, color: "#e85a18",
  ...
  dot: { width: 6, height: 6, borderRadius: "50%", background: "#e85a18", animation: "pulse 1.2s ease-in-out infinite" },
  ...
  tag: { padding: "4px 10px", fontSize: 12, borderRadius: 20 },
  caseItem: {
  ...
  caseTag: { padding: "3px 8px", fontSize: 13, color: "#555", background: "#f0f0f0", borderRadius: 4 },
  bookmarkBtn: {
  ...
  reasonTitle: { fontSize: 14, fontWeight: 500, color: "#e85a18", marginBottom: 6 },
